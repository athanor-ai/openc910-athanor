...
| Topic | State |
| --- | --- |
| Accepted evidence | Three accepted out-of-order module-local rows: `ct_prio` (the OoO priority arbiter), `ct_fifo` (the DEPTH=2 CIU FIFO), and `ct_rtu_pst_preg_entry` (RTU physical-register-status lifecycle recode), each with a replayable equivalence proof on its stated scope and a biting negative control. The RTU ROB-entry candidate (`ct_rtu_rob_entry`) has landed as a candidate. Two area/timing/OpenSTA-estimated-power candidates (`ct_prio`, `ct_rtu_pst_preg_entry`) have non-author replay and adversarial QA, but remain candidates with `customer_ready=false` until explicit promotion wording lands. A third metric candidate (`ct_rtu_pst_vreg_entry`) is packaged and pending non-author replay. |
...
| `ct_rtu_pst_vreg_entry` / lifecycle-state recode metrics | Candidate pending independent replay; `customer_ready=false` | Same mapped candidate netlist across axes: top-with-deps Sky130 area `3148.019200 -> 3057.932800`; OpenSTA max data-arrival `2.82 ns -> 2.81 ns` under the package SDC; OpenSTA estimated total power `1.28e-04 nW -> 1.24e-04 nW` under fixed global activity. Estimated power is not signoff power or workload-measured power. | Lifecycle-encoding relation proof under reset-first through passive debug wrappers. Proof red-control: `ct_rtu_pst_vreg_entry_gate_dbg_mutant.v` fails the same relation proof. Metric-only red-control reds all three axes: top-with-deps area `5287.571200`, max data-arrival `12.78 ns`, estimated total power `2.29e-04 nW`. | [`athanor_artifacts/rtu_pst_vreg_entry_area_timing_power_candidate1/`](athanor_artifacts/rtu_pst_vreg_entry_area_timing_power_candidate1/) |
...
1. Module-local rows across the OoO pipeline, RTU-first: the retire/reorder unit
   ...
   estimated-power receipts; it is pending independent replay.
