...
| Topic | State |
| --- | --- |
| Accepted evidence | Three accepted out-of-order module-local rows: `ct_prio` (the OoO priority arbiter), `ct_fifo` (the DEPTH=2 CIU FIFO), and `ct_rtu_pst_preg_entry` (RTU physical-register-status lifecycle recode), each with a replayable equivalence proof on its stated scope and a biting negative control. The RTU ROB-entry candidate (`ct_rtu_rob_entry`) has landed as a candidate. Three area/timing/OpenSTA-estimated-power candidates (`ct_prio`, `ct_rtu_pst_preg_entry`, `ct_rtu_pst_vreg_entry`) have non-author replay and adversarial QA, but remain candidates with `customer_ready=false` until explicit promotion wording lands. |
...
1. Module-local rows across the OoO pipeline, RTU-first: the retire/reorder unit
   ...
   that fail.
   A vector-register-status metric candidate is packaged at
   ...
   estimated-power receipts; independent replay has cleared and customer-ready
   promotion is not yet claimed.
