...
| `ct_rtu_pst_vreg` encoder family | Replay-confirmed parent-table metric scout | Replacing the shared `ct_rtu_encode_64` helper across the parent table improves selected Sky130 area `234172.089600 -> 233181.139200` and OpenSTA estimated power `9.41e-03 nW -> 9.39e-03 nW`, with max data-arrival flat at `8.30 ns`; helper equivalence, boundary-bit mutant, metric red-controls, and non-author replay all bite, but this remains scout-only without a parent-table theorem/authority | [`athanor_artifacts/rtu_pst_vreg_encoder_family_candidate1/`](athanor_artifacts/rtu_pst_vreg_encoder_family_candidate1/) |
...
| `ct_rtu_compare_iid` | Parent-integrated full-PPA scout | Replacing the shared wrapped-IID helper improves selected Sky130 area and OpenSTA estimated power in two LSU parents; `ct_lsu_spec_fail_predict` max data-arrival improves `4.73 ns -> 4.67 ns`, while `ct_lsu_pfu_sdb_cmp` worst max path stays flat at `8.14 ns` and its cross-clock reported path improves `7.00 ns -> 6.91 ns`; helper equivalence and metric red-controls bite, but direct parent proof routing currently tool-errors before verdict, so this remains scout-only | [`athanor_artifacts/ct_rtu_compare_iid_candidate1/`](athanor_artifacts/ct_rtu_compare_iid_candidate1/) |
