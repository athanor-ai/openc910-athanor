...
selected-flow measurements, equivalence or formal proof on the exact subject,
...
| Topic | State |
| --- | --- |
| Accepted evidence | Three accepted out-of-order module-local rows: `ct_prio` (the OoO priority arbiter), `ct_fifo` (the DEPTH=2 CIU FIFO), and `ct_rtu_pst_preg_entry` (RTU physical-register-status lifecycle recode), each with a replayable equivalence proof on its stated scope and a biting negative control. Three same-candidate-bound area/timing/OpenSTA-estimated-power packets (`ct_prio`, `ct_rtu_pst_preg_entry`, `ct_rtu_pst_vreg_entry`) are accepted for scoped module-local metric wording. The RTU ROB-entry candidate (`ct_rtu_rob_entry`) remains a candidate. |
| Accepted metric packet numbers | `ct_prio`: Sky130 area `158.902400 -> 60.057600`, OpenSTA max data-arrival `0.85 ns -> 0.67 ns`, OpenSTA estimated total power `1.08e-05 nW -> 2.89e-06 nW`. `ct_rtu_pst_preg_entry`: top-with-deps area `3510.867200 -> 3482.089600`, max data-arrival `3.23 ns -> 2.91 ns`, estimated total power `1.38e-04 nW -> 1.35e-04 nW`. `ct_rtu_pst_vreg_entry`: top-with-deps area `3148.019200 -> 3057.932800`, max data-arrival `2.82 ns -> 2.81 ns` (small delta, not a material timing claim), estimated total power `1.28e-04 nW -> 1.24e-04 nW`. OpenSTA power is a liberty estimate under fixed activity, not signoff or workload-measured power. |
...
1. Bounded RTL diff.
2. Selected-flow measurement on the recorded toolchain. Area-only rows must say
   that they are area-only; timing/power are required when those axes are part
   of the claim.
...
| Transform | Status | Measurement signal | Correctness / activity | Receipt |
| --- | --- | --- | --- | --- |
| `ct_prio` / priority-arbiter simplification | Accepted module-local artifact | Generic cells `22 -> 20`; Sky130 area `158.9 -> 93.8` (~41% on this block, verified real logic simplification, not dropped dead-logic); timing/power not claimed for this row | Yosys SAT temporal (k-)induction closes on `sel[1:0]` at depth 4 (unbounded, visible-output miter under a documented reset-first assumption); negative control: a broken candidate fails the same proof; internal-state boundary log-backed | [`athanor_artifacts/ct_prio/`](athanor_artifacts/ct_prio/) |
| `ct_prio` / one-flop priority encoding metrics | Accepted module-local metric packet; `customer_ready=true` for scoped area/timing/OpenSTA-estimated-power wording | Same mapped candidate netlist across all axes (one selected flow, not three synths): Sky130 area `158.902400 -> 60.057600`; OpenSTA max data-arrival `0.85 ns -> 0.67 ns` under the package SDC; OpenSTA estimated total power `1.08e-05 nW -> 2.89e-06 nW` under fixed global activity (`-activity 0.1 -duty 0.5`). Estimated power is a liberty estimate, not signoff power and not workload-measured power. | Two controls, cited separately. **Proof** negative-control: a functional mutant (`sel[0] := valid[0]`, dropping requester-1 priority over requester 0) **fails the same** reset-first `sel[1:0]` visible-output temporal-induction miter that the real candidate **passes**; the biting non-vacuity control. **Metric** red-control (separate, metric-only): reds all three axes: area `390.374400`, max data-arrival `2.45 ns`, estimated total power `1.84e-05 nW`. Both reproduced by independent cold-replay (Ronald proof-vocab + Quan adversarial, re-running the miter herself). | [`athanor_artifacts/ct_prio_area_timing_power_candidate1/`](athanor_artifacts/ct_prio_area_timing_power_candidate1/) |
| `ct_fifo` / DEPTH=2 pointer-representation specialization | Accepted module-local artifact | Sky130 area `711.9 -> 678.2` (~4.7% on this block); area-only, timing/power not claimed for this row | Exact visible-output equivalence (`fifo_pop_data[5:0]`, `fifo_pop_data_vld`, `fifo_full`, `fifo_empty`) proved **bounded through seq12** under a reset-first contract; an *unbounded* temporal-induction closure (k=1) holds over state-exposed copies that assert those outputs plus the create/pop-pointer, valid-vector and stored-data relation, gated by a scripted self-tested passivity check that the exact->debug bridge is instrumentation-only; negative control (inverted `fifo_full`) fails both the bounded miter and the relation miter | [`athanor_artifacts/ct_fifo/`](athanor_artifacts/ct_fifo/) |
| `ct_rtu_pst_preg_entry` / lifecycle-state recode | Accepted module-local relation packet | Sky130 local area `2677.6 -> 2648.8`; top-with-deps area `3510.9 -> 3482.1`; generic cells flat. Area-only, timing/power not claimed for this row | Exact visible-output BMC passes through seq12 under reset-first; no-reset check fails and records the reset boundary. Positive unbounded path is a passive-debug bridge plus lifecycle-encoding relation induction; the packet lifecycle mutant and an independent storage-path mutant both fail. Same-state bit equivalence is intentionally not claimed because the lifecycle state encoding changes; the `304` proven / `6` unproven split is retained only as a boundary artifact. Raw exact-output induction non-closure is also retained as a boundary artifact. | [`athanor_artifacts/rtu_pst_preg_entry_candidate1/`](athanor_artifacts/rtu_pst_preg_entry_candidate1/) |
| `ct_rtu_pst_preg_entry` / lifecycle-state recode metrics | Accepted module-local metric packet; `customer_ready=true` for scoped area/timing/OpenSTA-estimated-power wording | Same mapped candidate netlist across axes: top-with-deps Sky130 area `3510.867200 -> 3482.089600`; OpenSTA max data-arrival `3.23 ns -> 2.91 ns` under the package SDC; OpenSTA estimated total power `1.38e-04 nW -> 1.35e-04 nW` under fixed global activity. Estimated power is not signoff power or workload-measured power. | Reuses the accepted lifecycle relation proof under reset-first. Proof red-control: `ct_rtu_pst_preg_entry_gate_dbg_mutant.v` fails the same relation proof. Metric-only red-control: top-with-deps area `5711.728000`, max data-arrival `11.88 ns`, estimated total power `2.40e-04 nW`. Ronald and Quan independently replayed the proof, proof mutant, binding, and metric controls. | [`athanor_artifacts/rtu_pst_preg_entry_area_timing_power_candidate1/`](athanor_artifacts/rtu_pst_preg_entry_area_timing_power_candidate1/) |
| `ct_rtu_pst_vreg_entry` / lifecycle-state recode metrics | Accepted module-local metric packet; `customer_ready=true` for scoped area/timing/OpenSTA-estimated-power wording | Same mapped candidate netlist across axes: top-with-deps Sky130 area `3148.019200 -> 3057.932800`; OpenSTA max data-arrival `2.82 ns -> 2.81 ns` under the package SDC (small delta, not a material timing claim); OpenSTA estimated total power `1.28e-04 nW -> 1.24e-04 nW` under fixed global activity. Estimated power is not signoff power or workload-measured power. | Lifecycle-encoding relation proof under reset-first through passive debug wrappers. Proof red-control: `ct_rtu_pst_vreg_entry_gate_dbg_mutant.v` fails the same relation proof. Metric-only red-control reds all three axes: top-with-deps area `5287.571200`, max data-arrival `12.78 ns`, estimated total power `2.29e-04 nW`. Ronald and Quan independently replayed the proof, proof mutant, binding, and metric controls. | [`athanor_artifacts/rtu_pst_vreg_entry_area_timing_power_candidate1/`](athanor_artifacts/rtu_pst_vreg_entry_area_timing_power_candidate1/) |
...
a Sky130 area-only reduction `158.9 -> 93.8`; timing/power are not claimed for this
row.
...
the visible FIFO outputs. The selected-toolchain row records a Sky130 area-only
reduction `711.9 -> 678.2`; this is an area-only row, and timing/power are not
claimed.
...
- The useful signal is multi-axis: area, timing, power/toggle, proof,
  non-vacuity control, and replay all matter; current accepted rows are
  area-only unless a row explicitly says otherwise.
...
1. Module-local rows across the OoO pipeline, RTU-first: the retire/reorder unit
   ...
   ~0.9%), so it is not presented as an area result; timing/power are not
   claimed. Still open in this
   ...
   It carries a lifecycle onehot0 state-recode, selected Sky130 area-only improvement
   with timing/power not claimed, bounded exact-output evidence, a passive-debug
   bridge, closed lifecycle-encoding relation induction, and two relation mutants
   that fail.
   An accepted vector-register-status metric packet is packaged at
   [`athanor_artifacts/rtu_pst_vreg_entry_area_timing_power_candidate1/`](athanor_artifacts/rtu_pst_vreg_entry_area_timing_power_candidate1/)
   with same-candidate-bound area, OpenSTA max data-arrival, and OpenSTA
   estimated-power receipts; customer-ready wording is scoped to the module-local
   metric packet and keeps the signoff/workload-power boundary attached.
...
`ct_prio`, `ct_fifo`, and `ct_rtu_pst_preg_entry` are accepted fork-local
module packets under `athanor_artifacts/`, each replayable in place, while
