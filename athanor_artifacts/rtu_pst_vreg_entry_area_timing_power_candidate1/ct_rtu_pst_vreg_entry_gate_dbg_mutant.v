...
// &ModuleBeg; @27
module ct_rtu_pst_vreg_entry_gate(
  cp0_rtu_icg_en,
  cp0_yy_clk_en,
  cpurst_b,
...
output  [3 :0]  dbg_lifecycle_cur_state;
output  [6 :0]  dbg_iid;
output  [4 :0]  dbg_dstv_reg;
output  [5 :0]  dbg_rel_vreg;
output          dbg_wb_cur_state;
output          dbg_retire_inst0_iid_match;
output          dbg_retire_inst1_iid_match;
output          dbg_retire_inst2_iid_match;
// &Regs; @29
...
reg     [4 :0]  dstv_reg;                           
reg     [6 :0]  iid;                                
reg     [3 :0]  lifecycle_cur_state;                
reg     [3 :0]  lifecycle_next_state;               
reg     [5 :0]  rel_vreg;                           
...
parameter DEALLOC    = 4'b0001;
parameter WF_ALLOC   = 4'b0000;
parameter ALLOC      = 4'b0010;
parameter RETIRE     = 4'b0100;
...
assign sm_clk_en = rtu_yy_xx_flush
                   ...
                   || (lifecycle_cur_state[3:0] == RETIRE)
                      && retire_gateclk_vld
//                      && x_release_vld
                   || x_wb_vld
                   || (lifecycle_cur_state[3:0] == WF_ALLOC)
                      && create_vld
...
//==========================================================
assign reset_lifecycle_state[3:0] = (x_reset_mapped) ? RETIRE : DEALLOC;
...
begin
  if(!cpurst_b)
    lifecycle_cur_state[3:0] <= DEALLOC;
  else if(ifu_xx_sync_reset)
    lifecycle_cur_state[3:0] <= reset_lifecycle_state[3:0];
  else
    lifecycle_cur_state[3:0] <= lifecycle_next_state[3:0];
end
...
always @( retire_vld
       ...
       or rtu_yy_xx_flush
       or x_release_vld
       or lifecycle_cur_state[3:0]
       or create_vld)
begin
  case(lifecycle_cur_state[3:0])
    DEALLOC    : if(x_dealloc_vld && !rtu_yy_xx_flush)
                   lifecycle_next_state[3:0] = WF_ALLOC;
                 else
                   lifecycle_next_state[3:0] = DEALLOC;
    WF_ALLOC   : if(rtu_yy_xx_flush)
                   lifecycle_next_state[3:0] = DEALLOC;
                 else if(create_vld)
                   lifecycle_next_state[3:0] = ALLOC;
                 else
                   lifecycle_next_state[3:0] = WF_ALLOC;
    ALLOC      : if(rtu_yy_xx_flush)
                   ...
                   lifecycle_next_state[3:0] = RELEASE;
                 else if(retire_vld)
                   lifecycle_next_state[3:0] = RETIRE;
                 else
                   lifecycle_next_state[3:0] = ALLOC;
    ...
    RELEASE    : if(wb_cur_state_wb_masked)
                   ...
                   lifecycle_next_state[3:0] = RELEASE;
    default    :   lifecycle_next_state[3:0] = DEALLOC;
  endcase
// &CombEnd; @139
end
...
assign x_cur_state_alloc_release    = lifecycle_cur_state_alloc && lifecycle_cur_state_release;
...
assign dbg_lifecycle_cur_state = lifecycle_cur_state;
assign dbg_iid = iid;
assign dbg_dstv_reg = dstv_reg;
assign dbg_rel_vreg = rel_vreg;
assign dbg_wb_cur_state = wb_cur_state;
assign dbg_retire_inst0_iid_match = retire_inst0_iid_match;
assign dbg_retire_inst1_iid_match = retire_inst1_iid_match;
assign dbg_retire_inst2_iid_match = retire_inst2_iid_match;
