#!/usr/bin/env python3
"""Check that PST vreg-entry debug copies add observation-only ports."""
...
def _strip_debug_lines(text: str) -> str:
    ...
    for line in text.splitlines():
        ...
        if any(stripped == port + "," or stripped == port for port in DEBUG_PORTS):
            continue
        if any(stripped == port + ");" for port in DEBUG_PORTS):
            normalized.append(");")
            continue
...
def _run_checks(root: Path) -> tuple[bool, list[str]]:
    checks = [
        (root / "ct_rtu_pst_vreg_entry_gold.v", root / "ct_rtu_pst_vreg_entry_gold_dbg.v", "gold"),
        (root / "ct_rtu_pst_vreg_entry_gate_candidate.v", root / "ct_rtu_pst_vreg_entry_gate_dbg.v", "gate"),
    ]
    messages: list[str] = []
    ok = True
...
def _selftest(root: Path) -> tuple[bool, str]:
    with tempfile.TemporaryDirectory(prefix="ct_rtu_pst_passivity_") as tmp:
        ...
        target = tmp_root / "ct_rtu_pst_vreg_entry_gold_dbg.v"
