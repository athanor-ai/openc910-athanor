...
| `ct_rtu_compare_iid` candidate 1 | RTU/LSU shared IID ordering helper | parent-integrated full-PPA scout; parent proof route pending | Helper Sky130 area `142.636800 -> 127.622400`; `ct_lsu_spec_fail_predict` parent area `6576.307200 -> 6546.278400`, max data-arrival `4.73 ns -> 4.67 ns`, estimated power `3.79e-04 -> 3.78e-04 nW`; `ct_lsu_pfu_sdb_cmp` parent area `11684.956800 -> 11639.913600`, max path flat `8.14 ns -> 8.14 ns` with cross-clock path `7.00 ns -> 6.91 ns`, estimated power `5.92e-04 -> 5.91e-04 nW` | helper combinational equivalence `1` proven / `0` unproven; equality-boundary mutant leaves `1` cell unproven; metric red-control worsens both parent screens; direct parent same-state route currently tool-errors before verdict | [`ct_rtu_compare_iid_candidate1/`](ct_rtu_compare_iid_candidate1/) |
| `ct_rtu_pst_vreg` encoder-family candidate 1 | RTU vector-register-status parent table | candidate metric scout; independent replay pending | Same-candidate-bound parent Sky130 area `234172.089600 -> 233181.139200`; OpenSTA max data-arrival flat `8.30 ns -> 8.30 ns`; OpenSTA estimated total power `9.41e-03 nW -> 9.39e-03 nW` | helper-substitution equivalence for `ct_rtu_encode_64` proves; boundary-bit mutant leaves `1` cell unproven; metric red-control reds all three axes | [`rtu_pst_vreg_encoder_family_candidate1/`](rtu_pst_vreg_encoder_family_candidate1/) |
| `ct_rtu_pst_preg` encoder-family candidate 1 | RTU physical-register-status parent table | hard-negative metric scout; promotion rejected | Same-candidate-bound parent Sky130 area `384860.361600 -> 383196.265600`; OpenSTA estimated total power `1.51e-02 nW -> 1.50e-02 nW`; OpenSTA max data-arrival regresses `11.31 ns -> 11.35 ns` | helper-substitution equivalence for `ct_rtu_encode_96` proves; boundary-bit mutant leaves `1` cell unproven; metric red-control reds all three axes | [`rtu_pst_preg_encoder_family_candidate1/`](rtu_pst_preg_encoder_family_candidate1/) |
