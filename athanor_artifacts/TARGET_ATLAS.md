...
| `ct_idu_rf_fwd_preg` candidate 1 | IDU register-read forwarding helper | candidate helper metric scout; independent replay pending | Helper-local Sky130 area improves `2706.345600 -> 2382.284800`; OpenSTA max data-arrival improves `11.23 ns -> 6.10 ns`; OpenSTA estimated power improves `7.42e-05 nW -> 6.39e-05 nW` under fixed global activity | one-hot forwarding contract proves; lane-data proof mutant reds; data equivalence is scoped to exactly-one forwarding hit and `x_src_no_fwd` is checked unconditionally; no parent IDU/register-file claim | [`ct_idu_rf_fwd_preg_candidate1/`](ct_idu_rf_fwd_preg_candidate1/) |
