...
DEPS="gated_clk_cell.v ct_rtu_expand_32.v ct_rtu_expand_64.v ct_rtu_pst_vreg_entry.v"

"$YOSYS_BIN" -p "read_verilog -sv ct_rtu_encode_64_gold.v; rename ct_rtu_encode_64 gold; read_verilog -sv ct_rtu_encode_64_gate_candidate.v; rename ct_rtu_encode_64 gate; proc; opt; equiv_make gold gate equiv; hierarchy -top equiv; flatten; opt; equiv_simple; equiv_status -assert"   > "$OUT/helper_same_state_equiv.replay.log" 2>&1
...
"$YOSYS_BIN" -p "read_verilog -sv $DEPS ct_rtu_encode_64_gold.v ct_rtu_pst_vreg_gold.v; synth -top ct_rtu_pst_vreg; dfflibmap -liberty $LIBERTY; abc -liberty $LIBERTY; clean; stat -liberty $LIBERTY; write_verilog -noattr $OUT/ct_rtu_pst_vreg_gold.mapped.v"   > "$OUT/gold_map.replay.log" 2>&1
"$YOSYS_BIN" -p "read_verilog -sv $DEPS ct_rtu_encode_64_gate_candidate.v ct_rtu_pst_vreg_gate_candidate.v; synth -top ct_rtu_pst_vreg; dfflibmap -liberty $LIBERTY; abc -liberty $LIBERTY; clean; stat -liberty $LIBERTY; write_verilog -noattr $OUT/ct_rtu_pst_vreg_gate_candidate.mapped.v"   > "$OUT/gate_map.replay.log" 2>&1
"$YOSYS_BIN" -p "read_liberty -lib $LIBERTY; read_verilog ct_rtu_pst_vreg_metric_negative.mapped.v; hierarchy -top ct_rtu_pst_vreg; stat -liberty $LIBERTY"   > "$OUT/metric_negative_area.replay.log" 2>&1

echo "027de2cf7e4adc32ce491b9ab8be1d98bd7f6a50909eb9dfb33c2ca03e753607  $OUT/ct_rtu_pst_vreg_gold.mapped.v" | sha256sum -c -
echo "12278f70ac193d9345ce53c1380a68d344569498fb144c1fdedafde5cee16727  $OUT/ct_rtu_pst_vreg_gate_candidate.mapped.v" | sha256sum -c -
...
write_sta_tcl "$OUT/ct_rtu_pst_vreg_gold.mapped.v" "$OUT/gold_opensta.tcl"
write_sta_tcl "$OUT/ct_rtu_pst_vreg_gate_candidate.mapped.v" "$OUT/gate_opensta.tcl"
write_sta_tcl "$ROOT/ct_rtu_pst_vreg_metric_negative.mapped.v" "$OUT/metric_negative_opensta.tcl"
...
grep -qF "8.30   data arrival time" "$OUT/gold_opensta_area_timing_power.replay.log"
grep -qF "8.30   data arrival time" "$OUT/gate_opensta_area_timing_power.replay.log"
grep -qF "18.57   data arrival time" "$OUT/metric_negative_opensta_area_timing_power.replay.log"
grep -qF "Total                  6.26e-03   3.15e-03   9.00e-08   9.41e-03" "$OUT/gold_opensta_area_timing_power.replay.log"
