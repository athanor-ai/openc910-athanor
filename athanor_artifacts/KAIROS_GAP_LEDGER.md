...
| Shared helper wins need parent-screen batching, full PPA closure, and a parent proof route before promotion | `ct_rtu_compare_iid` wrapped-IID comparator scout | Kairos should recognize small reusable helpers, run a helper proof plus multiple parent screens, package same-candidate parent area/timing/power plus a metric red-control, then hold promotion until a supported parent proof route closes. The compare-IID helper is proof-clean and now parent-integrated full-PPA positive across two LSU parents, but direct parent same-state `equiv_make` tool-errors before verdict, so it remains a scout rather than a customer-ready result row. |
| Encoder-family transforms are width-sensitive | `ct_rtu_encode_32`; `ct_rtu_encode_64`; `ct_rtu_encode_96` | Kairos should batch related helpers by family but still score each width and each parent table independently. The 64-bit vreg table package keeps area/power signal with flat timing, the 96-bit preg table package is rejected by timing regression, and the 32-bit helper is a hard negative. A syntax pattern is not authority without per-table PPA and red controls. |
