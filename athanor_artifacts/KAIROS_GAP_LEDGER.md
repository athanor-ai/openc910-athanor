...
| Shared helper wins need parent-screen batching and full PPA closure before promotion | `ct_rtu_compare_iid` wrapped-IID comparator scout | Kairos should recognize small reusable helpers, run a helper proof plus multiple parent area screens, then hold promotion until same-candidate timing/power and a metric red-control are packaged. The compare-IID helper is proof-clean and area-positive in two LSU parents, but it remains a scout until full PPA closes. |
